...
class Quantifier(metaclass=ABCMeta):
    ...
    def fill(counts, min_len=0, max_len=scene_len):
        """
        Fills in missing counts arbitrarily and then pads the rest with the c_symbol.
        we assume the given counts meet the quantifier's requirements,
        in natural quantifiers this arises when:
        ...
        assert (0 <= min_len <= sum_counts <= max_len <= Quantifier.scene_len), "Illegal symbol counts given"
        
        # TODO: multinomial distribution is better (with filling in it shouldn't really matter)
        for symbol in symbols:
            if symbol not in counts:
                if symbol == c_symbol:
                    # assert again that we haven't generated a scene more that max_len length
        ...
        assert (sum(counts.values()) == Quantifier.scene_len),\
            "Symbol counts don't add up to scene length after filling in"
...
class Most(Quantifier):
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        ...
        assert (0 <= min_len <= max(min_len, 1) <= max_len <= Quantifier.scene_len), "Illegal scene length limits given"
...
class Some(Quantifier):
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        ...
        counts = defaultdict(int)
        counts[ab_symbol] = np.random.randint(max(min_len, 1), max_len + 1)
        return self.scene(counts, min_len, max_len)
...
class Few(Quantifier):
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        assert (0 <= min_len <= max(min_len, 2) <= max_len <= Quantifier.scene_len), "Illegal scene length limits given"

        counts = defaultdict(int)
        counts[ab_symbol] = np.random.randint(max(min_len, 2), max_len + 1)
        return self.scene(counts, min_len, max_len)
...
class No(Quantifier):
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        ...
        assert (0 <= min_len <= max(min_len, 1) <= max_len <= Quantifier.scene_len), \
            "Illegal scene length limits given"

        # assert (0 <= min_len <= max_len <= Quantifier.scene_len), "Illegal scene length limits given"

        counts = defaultdict(int)
        counts[ab_symbol] = 0
        counts[a_b_symbol] = np.random.randint(max(min_len, 1), high=max_len + 1)
        return self.scene(counts, min_len, max_len)
...
class All(Quantifier):
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        ...
        counts = defaultdict(int)
        counts[ab_symbol] = np.random.randint(max(min_len, 1), high=max_len + 1)
        counts[a_b_symbol] = 0
        return self.scene(counts, min_len, max_len)

    def quantification(self, counts):
        return counts[a_b_symbol] == 0  and counts[ab_symbol] > 0


class The(Quantifier):
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        """ can't generate scenes shorter than 1 """
        assert (0 <= min_len <= max(min_len, 1) <= max_len <= Quantifier.scene_len), "Illegal scene length limits given"
...
class Both(Quantifier):
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        """ can't generate scenes shorter than 2 """
        assert (0 <= min_len <= max(min_len, 2) <= max_len <= Quantifier.scene_len), "Illegal scene length limits given"
...
class N(Quantifier):
    ...
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        assert (0 <= min_len <= self.n <= max_len <= Quantifier.scene_len), "Illegal scene length limits given"
...
class ExactlyN(Quantifier):
    ...
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        assert (0 <= min_len <= self.n <= max_len <= Quantifier.scene_len),\
            "Illegal scene length limits given"
...
class MinMax(Quantifier):
    ...
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        assert (0 <= min_len <= self.mini <= self.maxi <= max_len <= Quantifier.scene_len),\
            "Illegal scene length limits given"
...
class Conjunction(Quantifier, metaclass=ABCMeta):
    ...
    def generate_scene(self, min_len=0, max_len=Quantifier.scene_len):
        ...
        return np.random.choice(self.quantifiers).generate_scene(min_len, max_len)
